...
struct Bank Bank = {{100, 100}, PTHREAD_MUTEX_INITIALIZER}; // global variable defined with mutex initializer
...
void *MakeTransactions() { // routine for thread execution
    ...
    for (i = 0; i < 100; i++) {
        ...
        pthread_mutex_lock(&Bank.lock); // Acquire the lock before accessing shared data
        ...
        pthread_mutex_unlock(&Bank.lock); // Release the lock after updating shared data
    }
    return NULL;
}
